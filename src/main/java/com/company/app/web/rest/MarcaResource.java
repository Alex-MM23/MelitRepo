...
import com.company.app.service.MarcaService;
...
/**
 * REST controller for managing {@link com.company.app.domain.Marca}.
 */
@RestController
...
public class MarcaResource {
    ...
    public ResponseEntity<Marca> createMarca(@RequestBody Marca marca) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/marcas/" + marca.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, marca.getId().toString()))
            .body(marca);
    }
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<Marca> updateMarca(@PathVariable(value = "id", required = false) final Long id, @RequestBody Marca marca)
    ...
    public ResponseEntity<Marca> partialUpdateMarca(@PathVariable(value = "id", required = false) final Long id, @RequestBody Marca marca)
        throws URISyntaxException {
