 ...
 */
public class MarcaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all marcas").get("/api/marcas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new marca")
                        .post("/api/marcas")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nombre\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_marca_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created marca").get("${new_marca_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created marca").delete("${new_marca_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Marca entity").exec(scn);
